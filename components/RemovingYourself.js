...
const RemovingYourself = (props) => {
    const navigation = useNavigation();

    const { removeClicked, id, accId } = props

    const communityContext = useContext(CommunityContext);
    const { removingMyself } = communityContext
    const handleDeletion = () => {
        console.log("bhae mein chalta hun")
        removingMyself(id, accId)
        navigation.pop()
        ...
        <View style={styles.myCommunities}>
            <View style={{
                ...
                <View style={{ flex: 1 / 4, flexDirection: 'row' }}>
                    <TouchableOpacity onPress={handleDeletion} style={{ flex: 1 / 2, justifyContent: 'center', alignItems: 'center' }}>
                        <Image
                            style={{
                                width: 40,
                                height: 40,
...
export default RemovingYourself;
