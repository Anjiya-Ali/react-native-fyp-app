...
const CreateCommunityPost = ({ description, setDescription, id }) => {
  const navigation = useNavigation();

  let attachments = [];
  const communityPostContext = useContext(CommunityPostContext);
  const { createCommunityPost, fileAttachments, setFileAttachments } =
  ...
  const notification = async () => {
    try {
      ...
      await CreateNotification(`${username} You have successfully created Community Post.`, "Community", "Community Post Creation", idd);
  ...
  const handlePostCreation = () => {
    ...
    if (!path && description.trim().length < 5) {
    ...
    if (path && description.trim().length === 0) {
    ...
    createCommunityPost(id, description, path);
    notification()
    navigation.pop();
  };
  return (
