...
import { setCommentsCount } from '../redux/action';
...
const PersonalContent = (props) => {
  ...
  const { creatorCard, setFileAttachments } = communityContext;
  const communityPostContext = useContext(CommunityPostContext);
  //likecommunityPost
  const { likeCommunityPost, allPosts } = communityPostContext;
  const {
    dp,
    name,
    date,
  ...
  const { likeMyPost, deleteCommunityPost, loggedIn } = personalPostContext;
  ...
  const notification = async () => {
    try {
      ...
      await CreateNotification(`"${username}" liked your Post".`, "MyPosts", "Post Liked", id2);
  ...
  const handleLikedMembers = () => {
    if (lik !== 0) {
      navigation.navigate("LikedMyPostMembers", {
        postId: postId,
      });
    }
  };
  ...
  const handleComments = () => {
    dispatch(setCommentsCount(comments));
    setHello(postId)
  ...
  const commentsCount = useSelector(state => state.community.commentsCount);

  // useEffect hook to respond to changes in commentsCount
  useEffect(() => {
    if (postId == hello && commentsCount != 0 && commState != commentsCount) {
      setCommState(commentsCount)
    }
  }, [commentsCount]);
  return (
      ...
      {del === false ? (
        ...
        >
          <View style={{ flex: 1 / 4, flexDirection: "row", marginTop: 10 }}>
            ...
            {creatorCard === loggedIn || memberIn === true ? (
              <View
                style={{
                  position: "absolute",
                  right: 20,
