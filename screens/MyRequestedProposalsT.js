...
const MyRequestedProposalsT = () => {
  const context = useContext(TopicRequestContext);
  const { GetRequestedProposalsT, WithdrawProposal } = context;
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchData = async () => {
        setFlag(false);
        const json = await GetRequestedProposalsT();
  ...
  const handleRejectContract = async (proposal_id) => {
...
export default MyRequestedProposalsT;