...
const MypendingInvitations = ({ route }) => {
  ...
  const { getPendingInvitations, pendingInvitations, withdrawInvitation } =
    context1;
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchData = async () => {
        setFlag(false);
        const json = await getPendingInvitations(additionalData);
        if (pendingInvitations) {
          setLocalRequests(json.memberInfo || []);
          setFilteredData(json.memberInfo || []);
        }
        if (json.memberInfo) {
          setLengthh(json.memberInfo.length);
        }
        setFlag(true);
      };
  ...
  return (
    <>
      {flag && (
        ...
        >
          <Header heading="Invited Connections" navigate="TeacherHomePage" />
