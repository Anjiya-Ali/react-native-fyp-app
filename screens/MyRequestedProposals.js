...
const MyRequestedProposals = () => {
  const context = useContext(TopicRequestContext);
  const { GetRequestedProposals, DeleteProposal } = context;
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchData = async () => {
        setFlag(false);
        const json = await GetRequestedProposals();
  ...
  const handleRejectContract = async (proposal_id, topic_request_id) => {
    setRequestId(topic_request_id);
  ...
  const handleDelete = async () => {
    try {
      await DeleteProposal(proposalId, requestId);
                        ...
                        >
                          ...
                          {proposal.status === "Requested" ? (
                            <>
                              <TouchableOpacity
                                onPress={() =>
                                  navigation.navigate("AcceptProposal", {
                                    additionalData: proposal.teacher_bid_id,
                                  })
                                }
                              >
...
export default MyRequestedProposals;