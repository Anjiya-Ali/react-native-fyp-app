...
const MyActiveProposalsT = () => {

  const context = useContext(TopicRequestContext);
  const { GetActiveProposalsT } = context;
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchData = async () => {
        setFlag(false)
        const json = await GetActiveProposalsT();
  ...
  const handlViewSingleProposal = async (id) => {
    try {
        navigation.navigate("ViewSingleProposalT", {
            additionalData: id,
        })
    } catch (error) {
  ...
  return (
    ...
    {flag && (
      ...
      <ScrollView
        style={{ flex: 1, flexDirection: flexD, backgroundColor: "#d9d9d9" }} refreshControl={
            ...
            {filteredData.map((proposal, index) => (
              <View key={index}>
                ...
                <View style={{
                    ...
                    <Text style={{ color: 'black', padding: 5, borderRadius: 10, fontWeight: 'bold', 
                                    backgroundColor: proposal.status === 'Requested' ? 'yellow' :
                                    proposal.status === 'Closed' ? 'red' :
                                    proposal.status === 'Active' ? 'green' : '#dc3545' }}>{proposal.status}</Text>
                </View>
                  <View style={styles.profileHeader}>
                    <View style={styles.profileTextContainer}>
                      <Text style={styles.userName}>{proposal.title}</Text>
                      <Text style={styles.userDetails}>{proposal.description.slice(0, 100)} ... </Text>
                    </View>
                  </View>
                  ...
                  <View style={{
                    ...
                    <View style={{ marginTop: 5, marginLeft: 36, width: "100%", flexDirection: 'row', justifyContent: 'flex-start', justifyContent: "space-between", marginRight: 36}}>
                        <TouchableOpacity
                            onPress={() => handlViewSingleProposal(proposal.teacher_bid_id)}
                            style={styles.viewAllButtonContainer}
                            >
                            <Text style={styles.viewAllButton}>  View Proposal  </Text>
                        </TouchableOpacity>
                    </View>
                  </View>
                </View>
      ...
      <View style={[styles.headerPosition1, { position: "relative" }]}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("TeacherProposals")}
        >
          <Image
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("MyRequestedProposalsT")}
        >
          <Image
            style={styles.ellipseImage}
            resizeMode="cover"
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("MyActiveProposalsT")}
        >
          <Image
            style={styles.ellipseImage}
            resizeMode="cover"
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate("MyClosedProposalsT")}
        >
          <Image
            style={styles.ellipseImage}
            resizeMode="cover"
...
export default MyActiveProposalsT;