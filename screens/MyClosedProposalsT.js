...
const MyClosedProposalsT = () => {
  const context = useContext(TopicRequestContext);
  const { GetClosedProposalsT, AddFeedbackStudent } = context;
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchData = async () => {
        setFlag(false);
        const json = await GetClosedProposalsT();
  ...
  const handleStarClick = (stars) => {
    ...
    updateDoneButtonStatus(stars);
  };

  const updateDoneButtonStatus = (stars) => {
    setIsDoneDisabled(stars === 0);
  };

  const handleSubmit = async (student_id, topic_request_id) => {
    await AddFeedbackStudent(
      student_id, 
      topic_request_id,
      selectedStars.toString(),
    );
    navigation.navigate("TeacherProposals");
  };
                          ...
                          <View>
                            {proposal.status === "Not Given" ? (
                              <>
                                ...
                                <TouchableOpacity
                                  onPress={() => handleSubmit(proposal.student_id, proposal.topic_request_id)}
                                  style={[
                                    styles.button,
                                ...
                                >
                                  <Text style={styles.done}>Give Feedback</Text>
                                </TouchableOpacity>
                              </>
                            ) : (
                              <></>
...
export default MyClosedProposalsT;