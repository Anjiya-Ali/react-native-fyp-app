...
const ManageStudents = () => {
  ...
  const { ManageStudents, removeStudent } = context;
  ...
  useFocusEffect(
    React.useCallback(() => {
      const fetchData = async () => {
        setFlag(false);
        const json = await ManageStudents();
        setAllStudents(json.students || []);
        setLocalStudents(json.students || []);
        setFilteredData(json.students || []);
        if (json.students) {
          setLengthh(json.students.length);
        }
  ...
  const handleRemoveStudent = async (studentId) => {
    ...
    setLocalStudents(
      localStudents.filter((request) => request.id !== studentId)
    );
    setFilteredData(filteredData.filter((request) => request.id !== studentId))
  ...
  const handleSearch = (text) => {
    ...
    const filtered = localStudents.filter(item =>
                      ...
                      <View style={styles.sectionContainer}>
                        <View style={styles.profileHeader}>
                          <Image
                            ...
                            resizeMode="cover"
                            source={{
                              uri: `${host}/${student.profile_picture}`,
                            }}
                          />
                        ...
                        >
                          <TouchableOpacity
                            onPress={() => handleBlockStudent(student.id)}
                          >
